...
function App(){
  ...
  async function queueAgentUpdates(){
    ...
    for(const serial of selectedSerials){
      const r=await fetch(api+'/jobs/agent-update',{
        ...
        body:JSON.stringify({serial,url:agentUpdate.url,sha256:agentUpdate.sha256||null,version:agentUpdate.version||null})
      });
  ...
  return <main>
    ...
    <section className="panel"><h2>Agent update vanuit Q-Central</h2><p className="muted">Selecteer devices, kies een release artifact en queue een <code>agent_update</code> job. De Q-Box agent haalt deze job op via heartbeat over de ZeroTier overlay.</p>
      <div className="formgrid">
        ...
        <label>SHA256<input placeholder="verplicht aanbevolen" value={agentUpdate.sha256} onChange={e=>setAgentUpdate({...agentUpdate,sha256:e.target.value})}/></label>
      </div>
